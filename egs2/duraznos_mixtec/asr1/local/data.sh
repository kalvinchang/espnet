...
mkdir -p ${DURAZNOS_MIXTEC}
if [ -z "${DURAZNOS_MIXTEC}" ]; then
    log "Fill the value of 'DURAZNOS_MIXTEC' of db.sh"
    exit 1
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    log "stage1: Download data to ${DURAZNOS_MIXTEC}"
    mkdir -p ${DURAZNOS_MIXTEC}
    wget -r -np -R "index.html*" ${URL}
    mv ${DURAZNOS_MIXTEC}/${DATASET_ROOT}/* ${DURAZNOS_MIXTEC}
fi

if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "stage2: Preparing data for duraznos_mixtec"
