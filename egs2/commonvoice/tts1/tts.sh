...
xvector_tool=speechbrain  # Toolkit for extracting x-vector (speechbrain, rawnet, kaldi)
xvector_model=speechbrain/spkrec-ecapa-voxceleb  # For only espnet, speechbrain, or rawnet

# still unclear if rawnet will be helpful for TTS; I can experiment with speechbrain or rawnet
# TODO: speechbrain overwrote old version of torch; modify installer to update speechbrain OR reinstall GPU version of torch with smaller version
...
# Training related
train_config="train_gst+xvector_tacotron2.yaml"    # Config for training.
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        log "STAGE 2 FORMAT WAV.SCP WAS COMMENTED"
