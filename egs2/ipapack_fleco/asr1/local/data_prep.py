...
def get_language(dataset: str, orig_split_name: str) -> str:
    match dataset:
        case "doreco":
            return orig_split_name.replace("_", "-")
        case "fleurs":
            return orig_split_name.split("-", 1)[0].replace("_", "-")

    raise ValueError(f"Unsupported dataset: {dataset!r}")


def get_utt_id(dataset, split, count, orig_split_name):
    return f"aaaaa_{get_language(dataset, orig_split_name)}_{dataset}_{split}_{count:025d}"
    ...
    for split, split_datasets in splits.items():
        for i, (dataset, orig_split_name) in tqdm(enumerate(split_datasets)):
            ...
            for cut in tqdm(cuts, miniters=1000):
                ...
                old_utt_id = cut.id
                utt_id = get_utt_id(dataset, split, utt_count, orig_split_name)
                utt_count += 1
